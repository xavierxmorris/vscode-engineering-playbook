...
> Every VS Code code excerpt in the guides now carries a 🔗 permalink pinned to this commit. See **[SOURCES.md](SOURCES.md)** for the full index of all 98 cited files.
...
### 2026-08-03 - Run 14
- **Verified** 170 citations, 90 SOURCES rows, 24 YAML fences at `7234ef0` - 0 problems; stated totals all correct at 90.
- **Added** ANALYSIS 6 "Loudly closing a registration window instead of dropping late arrivals" (`AsyncEmitter.fireAsync`, `event.ts` L1483-L1491 + L1501-L1505, delivery loop L1474-L1512). +2,819 bytes, inside budget. **The section-6 queue is now empty** - the next run must run a discovery sweep over `src/vs/base/common/` and `src/vs/platform/`.
- **Round 1 found eight defects.** The important one, which I had also found independently before the review returned: **the `catch`/`continue` at L1496-L1499 skips both the freeze *and* the await**, so if a listener throws, promises it already registered are never waited on - and a captured `waitUntil` can still append silently afterwards. My draft described the window as "exactly the synchronous body", which is only true on the normal-return path.
- Other corrections: "each listener" was overbroad (the loop stops early on cancellation, and receiving `waitUntil` does not mean calling it); delivery is **sequential** and that deserved saying explicitly; a late `waitUntil` throws **to its own caller** because `fireAsync` is no longer inside a `try` by then; the rejection claim was overstated - `onUnexpectedError` *ignores cancellation errors*, and a custom handler can still throw synchronously; and "costs one line" misattributed the protection, which requires the freeze **and** the `Object.isFrozen` guard together.
- **Elision annotation was dishonest** and is fixed: I had written "the intervening `listener(event)` call is elided" when L1492-L1500 also drops the event-object close and the entire catch/continue path. Now stated in full.
- **A clarity fix was adopted over a correctness reviewer's silence:** Round 2 flagged `promiseJoin` as unexplained magic in the excerpt; it is elided with a marker rather than described, since guessing its purpose would have been invention.
- **Rejected:** Round 2 also wanted the `onUnexpectedError` sentence deleted as "an implementation detail". Kept, because Round 1 had *corrected* rather than dismissed it - the rejection-handling behaviour is part of what makes the pattern safe. Correctness over tidiness.
### 2026-08-03 - Run 15 (discovery sweep; section-6 queue was empty)
- **Verified** 168 permalinks (all in-bounds), 24 YAML fences, 43 JSON fences at `7234ef0`. Re-derived every count: **18** PR jobs, **48** custom ESLint rules (50 `*.ts` in `.eslint-plugin-local` minus `index.ts`/`utils.ts`), **99** contrib folders, `workbenchTestServices.ts` **2,167 lines / 56 classes**. The 56 nearly became a false correction - a naive `^(export )?class ` regex returns **54** because two classes are indented; count with `^\s*`.
- **Pre-existing gap found and fixed:** `.github/workflows/pr-node-modules.yml` was cited in ANALYSIS 4.4 but had **no SOURCES row**, so the "complete index" was incomplete and the stated total was wrong. Found by cross-checking *cited paths against SOURCES rows* rather than only validating rows - a check no previous run had run. Row added; totals corrected everywhere (**90 -> 93** including this run's two new files).
- **Added** ANALYSIS 6 "Cleaning up a lock only if you still own it" (`SequencerByKey.queue`, `async.ts` L342-L354 - the complete method, byte-exact, no elision). Supporting citations: class/field L338-L340, test `async.test.ts` L1147-L1163, consumer `secrets.ts` L107 + L139-L140.
- **BUDGET EXCEEDED: +4,467 bytes vs the ~3,072 guideline.** Stated rather than gamed. ~1,300 bytes is the four-permalink citation line that Round 1 *required*; 830 bytes is SOURCES bookkeeping, part of which is the pre-existing-gap repair above, not new content.
- **I proved the central claim empirically before either review returned**, and kept the script: with the guard, max concurrency on one key is 1; without it, `start B | start C` overlap and it reaches 2. Also confirmed `finally(prev)` runs before `task2 START`, and that `.catch(() => { })` is what lets an already-queued follower still run (without it the follower never runs *and* inherits the predecessor's rejection).
- **Both reviewers independently caught the same factual error**, and Round 2 found it **without source access**: my sentence "the entry it would remove is the one the waiting task is queued behind" was backwards - when A finishes, the map holds **B's** promise, so a bare delete erases B's own entry. Replaced with a concrete A/B/C walkthrough matching both repros.
- **Round 1 corrections applied:** (1) the cited test does **not** cover a task already queued behind a rejection - it awaits the rejection at L1153-L1158 *before* re-queueing at L1161 - so the citation now says what the test does and does not prove; endpoint corrected L1162 -> **L1163**, the closing `});`. (2) "keeps the map from growing without bound" was false: `finally` never runs for a tail that never settles, so the guard bounds *idle* keys, not stuck ones. (3) "In any per-key async lock ... reference identity is enough" was overgeneralised - it holds only for serialisers that install a *distinct* promise per enqueue; designs reusing one long-lived entry need a token.
- **Two Round 1 nits rejected, both clarity-domain judgements where Round 2 explicitly ruled the other way and no fact was in dispute.** (a) Renaming the heading to "Cleaning up a per-key promise tail only if it is still current" - Round 2 called "lock" an appropriate, accessible metaphor, and house style asks for a plain-English mechanism name. (b) Adding a gloss for `ITask`/`TKey` - Round 2 said explicitly not to, as both are self-evident to the target reader. **The "correctness wins" rule applies to conflicts of fact, not of taste.**
- **Seam assessment: `src/vs/base/common/` is RICH, not exhausted.** The sweep returned six further line-verified candidates, queued in priority order: `arrays.ts` **L323-L335** (yields with `setTimeout` rather than a microtask because "any other delay function would starve I/O"); `async.ts` **L648-L661** (`disposableLongTimeout` re-arms against an absolute target to survive the 32-bit `setTimeout` overflow at ~24.8 days); `ipc.net.ts` **L1145-L1159** (three stale signals *and* a load estimator before declaring a socket dead, so an overloaded event loop is not mistaken for a dead peer); `event.ts` **L1372-L1386** (a nested `fire` drains the interrupted dispatch instead of overtaking it); `async.ts` **L1914-L1927** (`settled` - reject only after every sibling promise has settled); `async.ts` **L2333-L2347** (buffer producer output until the async-iterator executor installs real callbacks). Take one per run and re-verify independently - the sweep was not adversarially reviewed.
### 2026-08-03 - Run 16
- **Verified** 185 permalinks, 96 SOURCES rows, 24 YAML fences at `7234ef0` - **0 problems**, and the Run 15 cited-paths-vs-rows cross-check now passes (96 == 96). Counts re-derived: 18 PR jobs, 48 ESLint rules, 99 contrib folders, 2,167 lines / 56 classes.
- **Added** ANALYSIS 6 "Yielding with a timer so batched work does not block I/O" (`topAsync`, `arrays.ts` L323-L336, byte-exact, 14 lines). Taken from the Run 15 queue.
- **I caught my own off-by-one before any review saw it:** the excerpt is L323-**L336**, not L335 - I had counted 13 lines where the block is 14. The automated byte-exactness check found it, not reading.
- **I also disproved my own performance claim.** My first draft said the timer version "pays a clamped delay 399 times". A per-hop probe showed `setTimeout` costing ~15.4 ms/hop on this Windows host, **identical for no delay argument, `0` and `1`** - that is the OS timer granularity (~15.6 ms), not any JavaScript clamp. Round 1 independently reached the same conclusion and added the precise fact: Node applies a **1 ms default threshold** to an omitted timeout, and the browser's nested-timer 4 ms clamp is an unrelated mechanism. Absolute timings are now framed as host-dependent rather than quoted as constants.
- **Round 1 corrections applied:** (1) false absolutes removed - "cannot starve I/O" (heading), "I/O callbacks are delivered before the next pass", "delivers none of it"; a timer buys an *opportunity*, not an ordering guarantee. (2) The layering paragraph claimed "layering, not correctness" - wrong, `setImmediate` genuinely does not exist in browsers, so it *is* runtime correctness; the lib list was also incomplete (`ESNext.Disposable` and `DOM.Iterable` omitted), and the operative enforcement is `layersTypeCheck.ts` run as `valid-layers-check` in CI, not the tsconfig sitting on disk. (3) The annotation called a partial excerpt "the body" and mis-stated the omissions. (4) The batch-sizing advice optimised overhead only and could push chunks large enough to hurt latency.
- **Round 1 finding REJECTED on evidence.** It reported that the playbook misclassifies `postMessage` as scheduling a microtask, at two specific lines. I checked both: `PLAYBOOK-PHASE-1-2.md` L2778 already reads "It schedules a **task** (not a microtask)", and `VALIDATION.md` L94 records that exact error being caught and fixed in the original validation pass. The cited line numbers pointed at unrelated text. **Verify before acting on a review finding, including a good reviewer's.** Its underlying suggestion - a sentence reconciling this example with the test-runner section that *avoids* `setTimeout` - was worth keeping, and was added.
- **Round 2 (source-blind) restructured the dense measurement paragraph into two**, which was right, but its rewrite carried my mis-attributed "clamped delay" forward verbatim - it has no way to measure. Structure adopted, facts corrected independently.
- **BUDGET EXCEEDED: +5,851 bytes vs ~3,072, the largest overage so far.** Stated, not gamed. The example carries five permalinks and a four-part explanation because Round 1 required naming the operative enforcement chain (tsconfig -> `layersTypeCheck.ts` -> `valid-layers-check` -> `pr.yml`) rather than gesturing at a config file. Trimming would have meant dropping mandated citations.
- **Honest novelty note:** Round 1 judged this "established event-loop knowledge rather than novel theory". Kept because the *specific* trap - `await Promise.resolve()` used as a yield - is a common real-world mistake, and because the browser-layer constraint explaining the choice is genuinely non-obvious. A future run should weigh whether further event-loop material clears the non-obviousness bar.
- **Load-bearing check:** `topAsync` has exactly **one** production caller (`rawSearchService.ts` L265, batch 10,000). A repo-wide search also hits a large JSON perf fixture that contains the substring incidentally - not a call site.
- **Still queued** (from the Run 15 sweep, line-verified but not adversarially reviewed - re-verify independently): `async.ts` **L648-L661** (`disposableLongTimeout` re-arms against an absolute target to survive the ~24.8-day 32-bit `setTimeout` overflow); `ipc.net.ts` **L1145-L1159** (three stale signals *and* a load estimator before declaring a socket dead); `event.ts` **L1372-L1386** (a nested `fire` drains the interrupted dispatch instead of overtaking it); `async.ts` **L1914-L1927** (`settled` - reject only after every sibling settles); `async.ts` **L2333-L2347** (buffer producer output until the async-iterator executor installs real callbacks).
### 2026-08-03 - Run 17
- **Verified** 193 permalinks, 97 SOURCES rows, 24 YAML fences at `7234ef0` - 0 real problems. (The validator reports one benign hit, "15 cited", which is the phrase "the Run 15 cited-paths-vs-rows cross-check" in this log, not a stated total. Tighten the regex if it becomes noisy.)
- **Deliberately did NOT take the top queue item.** `disposableLongTimeout` (the ~24.8-day `setTimeout` overflow) is a good candidate, but Run 16 was already a `setTimeout` example and Round 1 had just warned that event-loop material is close to "established knowledge". Shipping a third timer example in a row would have been padding. It stays queued.
- **Added** ANALYSIS 6 "Not mistaking your own busy event loop for a dead peer" (`ipc.net.ts` L1145-L1155, byte-exact, 11 lines, plus five supporting ranges). `PersistentProtocol` will not declare a socket dead while `LoadEstimator` reports high load - and the estimator infers load from **how late its own one-second timer has been running**, not from CPU or memory.
- **I verified the off-by-one before the reviews:** `hasHighLoad()` is `load() >= 0.5` where `load = 1 - score/10`, so it is true when **five or fewer** of the ten stamps are fresh. Tabulated all eleven cases rather than reasoning about the boundary. Round 1 independently confirmed it.
- **Round 1 found four material defects.** (1) My "three clocks" description conflated two different paths: the unacknowledged-message path has **three** early returns before it (nothing outstanding, a check already scheduled, or reconnecting) and its first clock measures the **oldest pending message's write time**, not "the last acknowledgment"; the keep-alive path tests only **two** clocks, so "the same guard protects" it was misleading about the conditions even though the estimator gate is shared. (2) "the socket is only discarded" was wrong - this code fires `onSocketTimeout` and the connection layer reconnects; it never closes anything. (3) The estimator paragraph omitted that it **seeds ten synthetic stamps and therefore starts at load zero**, and my "the signal and the fault share a cause" was a false dichotomy: sleep, process suspension, timer throttling and wall-clock changes also stale the stamps, and blocking delays *JavaScript processing* of socket data rather than OS-level receipt. (4) "lets you distinguish" and "stops a local CPU spike" overstated a tunable heuristic.
- **Round 1 asked me to extend the excerpt through L1165 to show the actual operation. Declined** - that is 21 lines against a 15-line cap. Solved instead by annotating where the excerpt is cut and citing L1156-L1163 for what follows, which also answers Round 2's complaint that the block ends on an open brace.
- **Round 1 also proposed relabelling `**Prevents:**` to `**Mitigates:**`.** Declined on structural grounds: every example in the playbook uses `Prevents`, and the house anatomy defines that line as *naming the failure mode*, not asserting a guarantee. The overclaim was real but lived in "Adopt it", which was rewritten to "defer and re-check, not proof".
- **Round 2's proposed fix was rejected as fabrication.** To close the dangling block it suggested appending `// ...`, `}` and `}` to the excerpt - three lines that do not exist at L1145-L1155 and would have broken byte-exactness. Same trap as Run 13's invented `});`. Its other two fixes (state the healthy baseline "on a healthy loop all ten are"; split the over-packed final sentence) were adopted.
- **Load-bearing evidence:** only tests override `loadEstimator`; production takes `LoadEstimator.getInstance()` at L865. There is also a **third** guard site at `remoteAgentHostProtocolClient.ts` L1776, not cited here to avoid a further SOURCES row, but recorded as evidence this is live.
- **BUDGET EXCEEDED: +4,657 bytes vs ~3,072.** Stated, not gamed. Six permalinks into one file, because Round 1 required citing the early returns and the follow-on body rather than describing them.
- **Honest novelty note:** Round 1 calls this "established failure-detector practice, but a useful concrete implementation". That matches Run 16's verdict. **Two consecutive runs have now rated novelty as moderate rather than high** - the strongest material in `src/vs/base/common/` may be spent. The next run should weigh stopping against continuing.
- **Still queued** (line-verified, not adversarially reviewed): `async.ts` **L648-L661** (`disposableLongTimeout`, absolute-target re-arm across the 32-bit overflow; one production caller, `chatStatusEntry.ts` L265); `event.ts` **L1372-L1386** (nested `fire` drains the interrupted dispatch); `async.ts` **L1914-L1927** (`settled`); `async.ts` **L2333-L2347** (buffer producer output until the async-iterator executor installs real callbacks).
### 2026-08-03 - Run 18
- **Verified** 201 permalinks, 98 SOURCES rows, 24 YAML fences at `7234ef0` - **0 problems**. Counts re-derived: 18 PR jobs, 48 ESLint rules, 99 contrib folders, 2,167 lines / 56 classes. The validator's stated-total regex was tightened so it no longer matches prose like "the Run 15 cited-paths cross-check" (the false positive noted last run).
- **Added** ANALYSIS 6 "Draining the interrupted dispatch before starting a new one" (`event.ts` L1371-L1378 + L1384-L1386, both byte-exact, 14 lines including a declared marker line). The delivery loop advances its cursor **before** invoking each listener, so a listener that fires re-entrantly resumes at the next slot instead of the one already on the stack.
- **My reproduction and Round 1's disagreed, and both were right.** Mine (one-shot guard in the listener) produced `A(1) A(1) B(1)` - a **finite duplicate delivery**, so I had already softened my draft's "which recurses". Round 1 ran it **unguarded** and hit `RangeError: Maximum call stack size exceeded` at ~3,100 nested calls. The shipped text now states both outcomes and which condition produces each.
- **Round 1 found four material defects, one of which made my advice actively wrong.** (1) "The interrupted delivery completes before the new one starts" **misstates the call stack** - its trace `1a:start,1b,2a:start,2a:end,2b,1a:end` shows the *triggering callback has not returned*; only the remaining listener **slots** are drained. (2) The `UniqueContainer` fast path (L1392-L1399) means a **single-listener emitter bypasses the queue entirely**, even when handed a shared queue - and per L1174 an array never downgrades back, so my "two or more listeners" precondition needed restating as *array-backed*. (3) `end` is an **exclusive** upper bound (`i < end`), not "the last index to deliver", and "the post-increment is the whole trick" was false - it also requires `fire()` draining before enqueueing and `reset()` leaving `i === end` to stop the suspended outer loop. (4) **My "Adopt it" was wrong**: I told readers not to use a plain `for...of`, but Round 1's reproduction shows `for...of` yields `1a,2a,2b,1b` with **nothing duplicated or skipped** - it is simply depth-first interleaving, a legitimate choice. The advice now says to pick the semantics deliberately.
- **Round 2, with no source access, caught a real inaccuracy from the excerpt alone:** I wrote that the cursor "holds the listener array", but `dq.current!._listeners!` shows it holds the *emitter*, which holds the array. Corrected. Its other adopted fixes: the garden-path `Prevents` line, splitting the overloaded paragraph, and removing a duplicated explanation of the failure mode.
- **One Round 2 suggestion declined:** it wanted the unsupported "re-entrancy is the normal case" claim *justified* with an invented illustration (a `focus` handler triggering `layout`). Round 1 independently flagged the same sentence as unsupported but asked for it to be **weakened** instead. Took Round 1's route - weakening an overclaim beats inventing an example to prop it up.
- **BUDGET EXCEEDED: +4,736 bytes vs ~3,072.** Stated, not gamed. Seven permalinks, because Round 1 required citing the fast path, `reset()`, and the remainder of `fire()` rather than describing them.
- **NOVELTY IS NOW MODERATE THREE RUNS RUNNING** (16, 17, 18). Round 1's verdict here: "reentrancy-safe iteration is established, but this shared drain-before-new cursor/reset interaction is genuinely non-obvious." In Run 17 I committed to recommending a stop if this recurred, so: **the seam is thinning and this schedule should stop after at most one more run.**
- **Queue triage for whoever runs next.** Best remaining: `async.ts` **L2333-L2347** (an async-iterable source buffers values and errors emitted before its deferred executor installs the real callbacks - the only remaining candidate I would still call non-obvious). Weaker: `async.ts` **L648-L661** (`disposableLongTimeout`; good footgun, but would be the third timer example in four runs). **Pre-rejected, do not spend a run on it:** `async.ts` **L1914-L1927** (`settled`) - it is `Promise.allSettled` plus rethrow-first-error, and "`Promise.all` rejects early" is assumed knowledge, so it fails the non-obviousness bar.
### 2026-08-03 - Run 19 (FINAL RUN - schedule stopped, see below)
- **Verified** 207 permalinks, 98 SOURCES rows, 24 YAML fences at `7234ef0` - **0 problems**. Counts re-derived: 18 PR jobs, 48 ESLint rules, 99 contrib folders, 2,167 lines / 56 classes. No new SOURCES rows were needed - both cited files were already indexed - so the total stays at 98.
- **Added** ANALYSIS 6 "Buffering producer calls until a deferred stream writer exists" (`async.ts` L2320-L2330, byte-exact, 11 lines, plus five supporting ranges). `AsyncIterableSource` routes `emitOne`/`emitMany`/`reject` through mutable fields; until the deferred executor builds the real writer, temporary handlers capture into closure variables, and the executor then flushes and swaps the fields.
- **ROUND 1 RETURNED DO-NOT-SHIP** - the first outright rejection since Run 12, and correct. My draft's central sentence was **false on the early-error path**: `return` at L2322 happens *before* both the item flush and the three field assignments, so after an early rejection the handlers **stay in buffering mode permanently**. I had even reproduced that behaviour (`delivered=[] rejected=boom`) without noticing what it implied about the fields.
- **The rejection made the example better.** Round 1 argued the detail I had deliberately omitted - that the buffer variables are declared *after* the construction that closes over them, so the code is correct **only because the executor is deferred** - was the genuinely non-obvious part, and that omitting it made the "Adopt it" advice unsafe to follow. Promoting it to its own section changed the verdict on re-review from **MODERATE to HIGH** novelty. Lesson: when a reviewer says the interesting thing is missing, that is a stronger signal than a list of corrections.
- **I settled a factual disagreement between the two reproductions.** Round 1's sync-executor repro threw on `earlyItems`; mine threw on `earlyError`. Neither is authoritative about VS Code - the answer depends on statement order, and the executor's *first* access is `if (earlyError)` at L2320. Corrected to `earlyError` before Round 1's re-review independently confirmed it. The exact engine message is no longer quoted, per its second-round advice.
- **Second-round fixes applied:** the normal path was falsely unconditional (it only flushes and swaps **if no early error was buffered**); "if an error arrived first" misstated ordering (items may arrive first and still be suppressed); "initialised before anything can close over it" **states the wrong JavaScript rule** - creating a closure over an uninitialised binding is safe, *executing* it is not; "Whenever ... do not let that window silently no-op" was overbroad, since failing fast or exposing readiness are equally valid; and the writer is internal, not consumer-facing.
- **Round 2 (source-blind) contributed** the garden-path `Prevents` rewrite, the orphaned-jargon fix ("sink" was used only in "Adopt it", never defined), and the sharper "closure variables" over "local variables". **One of its suggestions was correctly refused:** it proposed explaining `this._deferred.p` as "simply keeps the executor alive and is not part of the buffering mechanism", flagged `[SPECULATIVE - VERIFY]`. It is a guess; the returned promise is what holds the iterable open until the owner calls `resolve()`/`reject()`. Not adopted as written.
- **BUDGET EXCEEDED: +4,310 bytes vs ~3,072.** Stated, not gamed. Seven permalinks and a four-part explanation, both required across two Round 1 passes.
- **Load-bearing:** two production construction sites (`extHostLanguageModels.ts` L48, `mainThreadLanguageModels.ts` L127). Thin but real - language-model response streaming.

## STOPPING THIS SCHEDULE

**Schedule #5 is stopped as of Run 19.** In Run 17 I committed to recommending a stop if novelty came back merely moderate again; it did in Run 18, and Run 19's first verdict was MODERATE too. That the rewrite reached HIGH does not change the trend - it took a DO-NOT-SHIP and a full reframing to get there, which is what a thinning seam looks like.

**Final state:** six dimensions, **eight** examples in section 6, **98** cited files, 207 permalinks, all verified against `7234ef0` with 0 outstanding problems.

**If anyone resumes this, read first:**
- The **build/test/CI surface is exhausted** (Run 11, evidence-backed). Do not re-mine it.
- `src/vs/base/common/` is now heavily mined: `async.ts`, `event.ts` and `arrays.ts` have each supplied multiple examples.
- **Pre-rejected, do not spend a run on it:** `async.ts` L1914-L1927 (`settled`) - it is `Promise.allSettled` plus rethrow-first-error, and "`Promise.all` rejects early" is assumed knowledge.
- **Weak but not rejected:** `async.ts` L648-L661 (`disposableLongTimeout`, the ~24.8-day `setTimeout` overflow). A real footgun, but it would be the fourth timer-adjacent example, and Runs 16-18 show adjacency drags the novelty verdict down.
- Genuinely untouched territory, if a future run wants a fresh seam rather than another `base/common` guard: `src/vs/platform/` beyond `ipc`, and `src/vs/base/parts/`.